...
function createTag() {
    createForm.post('/admin/sponsorship/tags', { preserveScroll: true, onSuccess: () => createForm.reset() });
}
...
function saveEdit(tag) {
    editForm.put(`/admin/sponsorship/tags/${tag.id}`, { preserveScroll: true, onSuccess: () => { editingTag.value = null; } });
}
...
function deleteTag() {
    useForm({}).delete(`/admin/sponsorship/tags/${deleteModal.value.id}`, { preserveScroll: true, onSuccess: () => { deleteModal.value = null; } });
}
</script>
...
<template>
    <AdminLayout>
        <template #header>
            <h2 class="text-lg font-semibold text-gray-900">Sponsorship Tags</h2>
        </template>
            ...
            <div class="bg-white rounded-2xl border border-gray-200 p-6">
                ...
                <form @submit.prevent="createTag" class="space-y-4">
                    <div>
                        ...
                        <input v-model="createForm.name" type="text" placeholder="Ej: Hot Lead / Key Account"
