...
<div class="email-card">
    ...
    <div class="body">
        <p class="greeting">Welcome, {{ $assistant->first_name }}!</p>

        <p class="text">
            You've been registered as an <strong>assistant</strong>
            @if($brandName || $designerName)
                for <strong>{{ $brandName ?? $designerName }}</strong>
            @endif
            {{ $eventName ? 'at ' . $eventName : '' }}.
            Download the Runway 7 app to access your event pass and credentials.
        </p>

        @if($eventName || $designerName || $brandName)
        <div class="info-box">
            @if($eventName)
            <div class="info-row">
            ...
            <div class="info-row">
                ...
                <span class="info-value">{{ $brandName ?? $designerName }}</span>
            </div>
            @endif
        </div>
        @endif
        ...
        <table class="credentials-table" cellpadding="0" cellspacing="0" border="0">
            <tr>
                ...
                <td class="credentials-value" style="background-color:#f7f7f7; font-weight:700; color:#111111; padding:14px 20px;">{{ $assistant->email }}</td>
            </tr>
            <tr>
